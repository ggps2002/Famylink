...
export function parseHourlyRate(str) {
    ...
    if (ratePlusMatch) {
        ...
        const rateRangeMatch = str.match(/\$(\d+(?:\.\d+)?)\s*-\s*\$(\d+(?:\.\d+)?)/);
        if (rateRangeMatch) {
    ...
    const shareRangeMatch = str.match(/\(Each family pays \$([\d.]+)\s*-\s*\$([\d.]+)\)/);
    if (shareRangeMatch) {
        result.minShare = parseFloat(shareRangeMatch[1]);
        result.maxShare = parseFloat(shareRangeMatch[2]);
    } else {
        const sharePlusMatch = str.match(/\(Each family pays \$(\d+(?:\.\d+)?)\+\)/);
        if (sharePlusMatch) {
...
export function findMatchingRate(hourlyRate) {
    const rangeData = [
        { name: '$10 - $15 per hour (Each family pays $5 - $7.50)', val: '$10 - $15 per hour (Each family pays $5 - $7.50)' },
        { name: '$15 - $20 per hour (Each family pays $7.50 - $10)', val: '$15 - $20 per hour (Each family pays $7.50 - $10)' },
        { name: '$20 - $25 per hour (Each family pays $10 - $12.50)', val: '$20 - $25 per hour (Each family pays $10 - $12.50)' },
        { name: '$25 - $30 per hour (Each family pays $12.50 - $15)', val: '$25 - $30 per hour (Each family pays $12.50 - $15)' },
        { name: '$30 - $35 per hour (Each family pays $15 - $17.50)', val: '$30 - $35 per hour (Each family pays $15 - $17.50)' },
        { name: '$35 - $40 per hour (Each family pays $17.50 - $20)', val: '$35 - $40 per hour (Each family pays $17.50 - $20)' },
        { name: '$40+ per hour (Each family pays $20+)', val: '$40+ per hour (Each family pays $20+)' },
    ];
    ...
    if (!hourlyRate.max && !hourlyRate.maxShare) {
        const target = `$${format(hourlyRate.min)}+ per hour (Each family pays $${format(hourlyRate.minShare)}+)`;
        return rangeData.find(option => option.val === target)?.name || "N/A";
    } else {
        const target = `$${format(hourlyRate.min)} - $${format(hourlyRate.max)} per hour (Each family pays $${format(hourlyRate.minShare)} - $${format(hourlyRate.maxShare)})`;
        return rangeData.find(option => option.val === target)?.name || "N/A";
    }
}
...
export const step3Data = [
    { name: "Montessori Attachment parenting RIE", val: "Montessori Attachment parenting RIE" },
