...
export default function Header({ join }) {
    ...
    return (
        <>
            {join ? (
                <div className="top-0 z-50 sticky flex items-center bg-white mb-3 w-full h-20 font-semibold text-xl padd-res">
                    <NavLink to="/" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}>
                        <img src={logo} className="w-40 object-contain" alt="logo" />
                    </NavLink>
                </div>
            ) : (
                <header className="top-0 z-50 sticky w-full bg-white shadow-md">
                    ...
                    >
                        <div className="px-4 pb-2 pt-2">
                            {navItems.map(({ to, label }) => (
                                <HeaderLink key={to} to={to} label={label} onClick={closeNavbar} />
                            ))}
