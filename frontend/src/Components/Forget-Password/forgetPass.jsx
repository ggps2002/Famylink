...
import { sendOtpThunk, verifyOtpThunk, resendOtpThunk } from '../Redux/forgetPassword'
export default function ForgetPass() {
    ...
    const handleSubmit = async values => {
        try {
            if (step == 0) {
                const { data } = await dispatch(sendOtpThunk({ email: values.email })).unwrap()
                ...
                const { data } = await dispatch(verifyOtpThunk({ otp: values.otp, newPassword: values.password })).unwrap()
                setEmail(null)
                setTimeLeft(0)
                form.resetFields();
                navigate('/login')
                            ...
                            <Form name='loginForm' layout='vertical' onFinish={handleSubmit}>
                                ...
                                }
                                {
                                    ...
                                    <>
                                        ...
                                        <div>
                                            <p className='mb-1 text-xl capitalize Classico'>New Password</p>
                                            <Form.Item
                                                name='password'
                                                rules={[{ required: true, message: '' }]}
                                                style={{ margin: 0 }}
                                ...
                                <div className='my-5 text-center'>
                                    <div className='my-5 text-center'>
                                        <Button
                                            ...
                                            htmlType={(step == 1 && timeLeft == 0) ? 'button' : 'submit'}
                                        >
                                            {step == 0 && 'Request'}
                                    ...
                                    <p className='mt-2 mb-1 font-normal text-base already-acc'>
                                        New to Famylink? <NavLink to='/joinNow'>
