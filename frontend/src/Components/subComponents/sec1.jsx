...
export default function Sec1({ pic, head, preHead }) {
  return (
    <div className='flex justify-between items-center'>
      <div>
        ...
        >
          <p className='Classico sec1-head'>{head}</p></AnimatedWrapper>
        <p className='Elliana-Samantha sec1-pre-head'>{preHead}</p>
        ...
        <p className='font-normal text-base already-acc'>
          <NavLink className='hover:text-blue-600 transition-colors duration-300 cursor-pointer' to='/login' onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}>Already have an account? Log in</NavLink>
        </p>
      </div>
