...
export default function HireStep4({ formRef, head, textAreaHead, subHead, data, checkBox, inputName, defaultVal, inputNot, subHead1, subHead2 }) {
    ...
    return (
        <div>
            ...
            <div className={`flex items-center flex-col justify-center ${!subHead2 ? 'my-10' : 'mb-10'}`}>
                <div>
                    <p className={`Classico text-xl !text-wrap mb-2  ${(data.length > 3) ? '' : 'input-text'}`}>{subHead1}</p>
                    <Form form={form} name="validateOnly" autoComplete="off">
                        {checkBox ? (
                            <div className='flex flex-col justify-center items-center'>
                                <Form.Item name="option">
                                    ...
                                    >
                                        <div className={`${data.length > 3 && 'grid grid-cols-1 lg:grid-cols-2 gap-4'}`}>
                                            {data.map((v, i) => (
                                                <div key={i}>
                                                    <div className='input-width bg-white rounded-3xl py-4 px-4 mb-4'>
                                                        <div className='flex justify-between'>
                                                            <span>{v.name}</span>
                                                            <Checkbox value={v.val ? v.val : toCamelCase(v.name)} />
                                                        </div>
                                                    </div>
                                                    {v.textArea && (
                                                        <InputTextArea
                            ...
                            <div className={`${data.length > 3 && 'grid grid-cols-1 lg:grid-cols-2 gap-x-2'}`}>
                                {
                                    data.map((v, i) => (
                        ...
                        {checkBox && (
                            ...
                            >
                                {selectedValues.length === allValues.length ? 'Deselect All' : 'Select all that apply'}
                            </p>
                        )}
