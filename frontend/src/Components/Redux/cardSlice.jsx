...
export const saveCardThunk = createAsyncThunk(
    ...
    async (paymentMethod, { getState, rejectWithValue }) => {
        try {
            ...
            const { data, status } = await api.post("/payment/stripe/save-card",
...
export const getSaveCardThunk = createAsyncThunk(
    ...
    async (_, { getState, rejectWithValue }) => {
        try {
            ...
            const { data, status } = await api.get("/payment/stripe/get-save-cards", // Ensure this matches your backend
                {
                    headers: {
                        Authorization: `Bearer ${accessToken}`, // Send the token for authentication
                    },
...
export const deleteCardThunk = createAsyncThunk(
    ...
    async (paymentMethodId, { getState, rejectWithValue }) => {
        try {
            ...
            const { data, status } = await api.delete(`/payment/stripe/delete-card/${paymentMethodId}`, {
                headers: {
                    Authorization: `Bearer ${accessToken}`, // Send the token for authentication
                },
            });
...
export const processWebhook = createAsyncThunk(
    ...
    async (eventData, { rejectWithValue }) => {
        try {
            ...
            const { data, status } = await api.post("/payment/stripe/webhook", eventData, { headers });

            return { data, status }; // Return response data after processing webhook
        } catch (error) {
            console.error("Webhook error:", error);
            return rejectWithValue(error.response?.data || "Failed to process webhook");
        }
    }
);
