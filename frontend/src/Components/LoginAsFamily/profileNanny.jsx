...
import { HeartOutlined, CheckOutlined, HeartFilled, CloseOutlined } from '@ant-design/icons'
...
import { requestThunk, statusThunk, withDrawThunk } from '../Redux/bookHireSlice';
...
export default function ProfileNanny() {
        ...
        try {
            ...
            if (fetchedStatus && fetchedStatus.length > 0) {
                ...
                if (currentStatus.status === 'pending') {
                    newButtonText = currentStatus.userRole === 'requester' ? 'With draw' : 'Accept Request';
                ...
                // }
                else if (currentStatus.status === 'completed' && !currentStatus?.booking?.familyReview) {
    ...
    const handleMessage = async () => {
        try {
            ...
            const { data, status } = await dispatch(createChatThunk({ participants })).unwrap();
    ...
    return (
        <div className="padding-navbar1 Quicksand">
            ...
            <div className='shadow-xl border-[1px] border-[#D6DDEB] bg-white my-8 px-6 py-4 rounded-2xl text-center'>
                <div onClick={favourite} className='flex justify-end items-center gap-x-1 text-red-600 cursor-pointer'>
                    {
                    ...
                    <div>
                        {
                            selectedNanny?.imageUrl ?
                                <img className='mx-auto rounded-full w-24 object-contain' src={selectedNanny?.imageUrl} alt="img" />
                                : <Avatar
                                    className='rounded-full text-black' size='96' color={'#38AEE3'}
                                    name={
                                        selectedNanny?.name
                        ...
                        <p className='my-2 font-bold lg:text-3xl text-2xl'>{selectedNanny?.name}</p>
                         <p className='font-semibold text-lg'>{selectedNanny?.location?.format_location}</p>
                        {/* <p className='font-semibold text-lg'>{selectedNanny?.zipCode && `Zip Code: ${selectedNanny?.zipCode}`}</p> */}
                        {
                            ...
                            <p className='mb-2 text-lg'>{selectedNanny?.gender} {selectedNanny?.age && `| Age: ${selectedNanny?.age}`}</p>
                        }
                        ...
                        <Ra points={selectedNanny?.averageRating ? selectedNanny?.averageRating : 0} size={16} />
                        <div className='my-4'>
                            <button
            ...
            <div className='border-[1px] border-[#D6DDEB] bg-white p-4 rounded-2xl'>
                ...
                <div className='flex flex-wrap justify-left gap-x-10 gap-y-5'>
                    {days.map((day, index) => {
                        const dayData = selectedNanny?.additionalInfo[3]?.value[day];
                        return (
                            <div
                                key={index}
                                className={`pr-8 ${index < days.length - 1 ? 'schdule-Border' : ''}`}
                            >
                                    ...
                                    <>
                                        <p>
                                            ...
                                            <span className='font-bold'>
                                                {new Date(dayData.start).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true })}
                                            </span>
                                        </p>
                                        <p>
                                            End{' '}
                                            <span className='font-bold'>
                                                {new Date(dayData.end).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true })}
                                            </span>
                                        </p>
                                    </>
                                ) : (
            ...
            <div className='border-[1px] border-[#D6DDEB] bg-white my-8 p-4 rounded-2xl'>
                ...
                <div className='flex flex-wrap justify-between gap-x-10 gap-y-5'>
                    {
                        Object.entries(selectedNanny.additionalInfo.find(info => info.key === "salaryExp")?.value)?.map(([key, value], i) => (
            ...
            <div className='flex flex-wrap gap-x-4 gap-y-6 w-full'>
                <div className='border-[1px] border-[#D6DDEB] bg-white p-4 rounded-2xl width-div'> {/* 75% width */}
                    <p className='mb-2 font-bold text-2xl'>About Me</p>
                    <p className='leading-5'>{selectedNanny?.additionalInfo.find(info => info.key === "jobDescription")?.value}</p>
                </div>
                <div className='border-[1px] border-[#D6DDEB] bg-white p-4 rounded-2xl width-2div'> {/* 25% width */}
                    <p className='mb-2 font-bold text-2xl'>Looking For</p>
                    <div>
                        <p className='text-base'><span className='font-semibold text-lg Quicksand'>Availability:</span> {selectedNanny?.additionalInfo.find(info => info.key === "avaiForWorking")?.value?.option}</p>

                        <p className='text-base'><span className='font-semibold text-lg Quicksand'>Start:</span> {selectedNanny?.additionalInfo.find(info => info.key === "availability")?.value?.option}</p>

                        <p className='text-base'><span className='font-semibold text-lg Quicksand'>Age group:</span> {
                            selectedNanny?.additionalInfo.find(info => info.key === "ageGroupsExp")?.value?.option.map((v, i) => (
                                <span key={i}>
                                    {customFormat(v).split(' ')[0]}
                                    {i < selectedNanny?.additionalInfo.find(info => info.key === "ageGroupsExp")?.value?.option.length - 1 ? ', ' : ''}
                                </span>
                            ))
                        }</p>
                    </div>
            ...
            <div className='flex flex-wrap gap-x-4 gap-y-6 my-8 w-full'>
                <div className='border-[1px] border-[#D6DDEB] bg-white p-4 rounded-2xl width-div'> {/* 75% width */}
                    ...
                    <div>
                        <p className='text-base'>
                            <span className='font-semibold text-lg Quicksand'>Certifications: </span>
                            {
                                selectedNanny?.additionalInfo.find(info => info.key === "certification")?.value?.option.map((v, i) => (
                                    <span key={i}>
                                        {customFormat(v)}
                                        {i < selectedNanny?.additionalInfo.find(info => info.key === "certification")?.value?.option.length - 1 ? ', ' : ''}
                                    </span>
                                ))
                            }
                        </p>
                        <p className='text-base'><span className='font-semibold text-lg Quicksand'>Languages: </span>
                            {
                                selectedNanny?.additionalInfo.find(info => info.key === "language")?.value?.option.map((v, i) => (
                                    <span key={i}>
                                        {customFormat(v)}
                                        {i < selectedNanny?.additionalInfo.find(info => info.key === "language")?.value?.option.length - 1 ? ', ' : ''}
                                    </span>
                                ))
                            }
                        </p>
                        <p className='text-base'>
                            ...
                            {`${customFormat(
                                user.additionalInfo.find(info => info.key === 'ableToCook')?.value?.option || ''
                            )}, ${customFormat(
                                user.additionalInfo.find(info => info.key === 'helpWithHousekeeping')?.value?.option || ''
                            )}`}
                        </p>
                    </div>
                </div>
                <div className='border-[1px] border-[#D6DDEB] bg-white p-4 rounded-2xl width-2div'>
                    ...
                    <div>
                        <p className='font-semibold text-lg Quicksand'>{selectedNanny?.additionalInfo.find(info => info.key === "experience")?.value?.option} of experience caring for:</p>
                        {
                            selectedNanny?.additionalInfo.find(info => info.key === "ageGroupsExp")?.value?.option.map((v, i) => (
                                <p className='text-base'>
            ...
            <div className='flex flex-wrap gap-x-4 gap-y-6 my-8 w-full'>
                                    ...
                                    <div>
                                        {
                                            ...
                                            <p className='font-bold text-4xl text-center Quicksand'>{selectedNanny?.averageRating}</p>
                                        }

                                        <Ra points={selectedNanny?.averageRating ? selectedNanny?.averageRating : 0} size={8} />
                                        <p style={{ fontSize: 8 }}>{selectedNanny?.reviews?.length} Reviews</p>
                                    </div>
                                    <div>
                ...
                <div className='border-[1px] border-[#D6DDEB] bg-white p-4 rounded-2xl h-40 width-2div'>
                    ...
                    <div className='flex'>
                        ...
                        {
                            selectedNanny?.verified?.nationalIDVer == 'true' ? <CheckOutlined /> : <CloseOutlined />
                        }
                    </div>
                    ...
                    <div className='flex'>
                        ...
                        {
                            selectedNanny?.verified?.emailVer ? <CheckOutlined /> : <CloseOutlined />
                        }
                    </div>
                </div>
            </div>
