    ...
    if (currentStep == 1) {
      ...
      jobFormRef.current
        ...
        .then(values => {
          ...
          if (Array.isArray(values.option) && values.option?.length > 0) {
            // If form is valid, submit it and move to the next step
            // console.log(values)
            let updatedValues = { ...formValues, specificDuties: values.option, ...(values.otherPreferences && { specificDutiesSpecify: values.otherPreferences }) }; // Merge with previous values
            setFormValues(updatedValues);
            jobFormRef.current.resetFields()
            stepRef.current?.next()
            window.scrollTo({ top: 0, behavior: "smooth" })
      ...
      jobFormRef.current
        ...
        .then(values => {
          if (Array.isArray(values.option) && values.option?.length > 0) {
            // If form is valid, submit it and move to the next step
            // console.log(values)
            let updatedValues = { ...formValues, specificCare: values.option, ...(values.otherPreferences && { specificCareSpecify: values.otherPreferences }) }; // Merge with previous values
            setFormValues(updatedValues);
            jobFormRef.current.resetFields()
            stepRef.current?.next()
            window.scrollTo({ top: 0, behavior: "smooth" })
      ...
      jobFormRef.current
        ...
        .then(values => {
          if (Array.isArray(values.option) && values.option?.length > 0) {
            // If form is valid, submit it and move to the next step
            // console.log(values)
            let updatedValues = { ...formValues, duration: values.option, ...(values.otherPreferences && { durationSpecify: values.otherPreferences }) }; // Merge with previous values
            setFormValues(updatedValues);
            jobFormRef.current.resetFields()
            stepRef.current?.next()
            window.scrollTo({ top: 0, behavior: "smooth" })
      ...
      jobFormRef.current
        ...
        .then(values => {
          if (Array.isArray(values.option) && values.option?.length > 0) {
            // If form is valid, submit it and move to the next step
            let updatedValues = { ...formValues, expAndQua: values.option, ...(values.otherPreferencesCertifications && { expAndQuaSpecify: values.otherPreferencesCertifications }) }; // Merge with previous values
            setFormValues(updatedValues);
            jobFormRef.current.resetFields()
            stepRef.current?.next()
            window.scrollTo({ top: 0, behavior: "smooth" })
        ...
        .then(values => {
          if (values.option) {
            ...
            let updatedValues = {
              ...
              ...(cleanData.otherPreferences && { availabilitySpecify: cleanData.otherPreferences }) // Correct conditional property assignment
            };
      ...
      jobFormRef.current
        ...
        .then(values => {
          if (Array.isArray(values.option) && values.option?.length > 0) {
            // If form is valid, submit it and move to the next step
            let updatedValues = { ...formValues, addSkills: values.option, ...(values.otherPreferences && { addSkillsSpecify: values.otherPreferences }) }; // Merge with previous values
            setFormValues(updatedValues);
            jobFormRef.current.resetFields()
            stepRef.current?.next()
            window.scrollTo({ top: 0, behavior: "smooth" })
      ...
      jobFormRef.current
        ...
        .then(values => {
          ...
          if (Array.isArray(values.option) && values.option?.length > 0) {
            // If form is valid, submit it and move to the next step
            let updatedValues = { ...formValues, personalFit: values.option, ...(values.otherPreferences && { personalFitSpecify: values.otherPreferences }) }; // Merge with previous values
            setFormValues(updatedValues);
            jobFormRef.current.resetFields()
            stepRef.current?.next()
            window.scrollTo({ top: 0, behavior: "smooth" })
