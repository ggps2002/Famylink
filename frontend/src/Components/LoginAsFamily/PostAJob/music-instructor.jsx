      ...
      jobFormRef.current
        ...
        .then(values => {
          ...
          if (Array.isArray(values.option) && values.option?.length > 0) {
            // If form is valid, submit it and move to the next step
            // console.log(values)
            let updatedValues = { ...formValues, typeOf: values.option, ...(values.otherPreferences && { typeSpecify: values.otherPreferences }) }; // Merge with previous values
            setFormValues(updatedValues);
            jobFormRef.current.resetFields()
            stepRef.current?.next()
            window.scrollTo({ top: 0, behavior: "smooth" })
      ...
      jobFormRef.current
        ...
        .then(values => {
          if (values.option) {
            ...
            let updatedValues = { ...formValues, skillsLevel: cleanData.option }; // Merge with previous values
            setFormValues(updatedValues);
            jobFormRef.current.resetFields()
            stepRef.current?.next()
            window.scrollTo({ top: 0, behavior: "smooth" })
      ...
      jobFormRef.current
        ...
        .then(values => {
          ...
          if (values.option) {
            ...
            let updatedValues = { ...formValues, sessionTime: cleanData.option }; // Merge with previous values
            setFormValues(updatedValues);
            jobFormRef.current.resetFields()
            stepRef.current?.next()
            window.scrollTo({ top: 0, behavior: "smooth" })
    ...
    switch (currentStep) {
      ...
      case 6: return (
        <HireStep4
          formRef={jobFormRef || {}}
          inputNot={true}
          subHead1={'Do you prefer lessons at your home, the instructor’s studio, or online?'}
          data={step5Data}
        />
      );
      case 7:
      ...
      case 9: return (
        <HireStep4
          ...
          formRef={jobFormRef || {}}
          inputNot={true}
          subHead1={"Do you have a preference for the instructor's teaching style?"}
          data={step8Data}
        />
      );
      case 10:
            ...
            >
              <div>
                <p className='mt-10 mb-1 text-xl Classico'>
                  Do you have any special requirements or <br className="max-lg:hidden" />preferences for the music instructor?
                </p>
                <Form.Item
