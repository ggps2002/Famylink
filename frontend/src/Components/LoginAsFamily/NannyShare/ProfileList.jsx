...
export default function ProfileList({
    ...
    const { data, pagination, isLoading } = useSelector(state =>
        state.postNannyShare
    )
    ...
    return (
        <div className='flex flex-col w-full px-0 lg:px-4 2xl:px-8'>
            <div className={"grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 2xl:grid-cols-4"}>
                {isLoading ? (
                    <Loader />
                ) : data?.length > 0 ? (
                    data.map((profile) => (
